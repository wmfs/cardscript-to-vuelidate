...
module.exports = function (cardscript) {
  ...
  function parseElement (element, idx) {
    if (element.id) {
      ...
      if (element.type === 'Input.Signature' && element.allowNameInput) {
        json[element.id + 'Name'] = json[element.id]
      }
    ...
    for (const [type, path] of Object.entries(collections)) {
      if (element.type === type) {
        const dataSet = dottie.get(element, path)

        if (Array.isArray(dataSet)) {
          dataSet.forEach(parseElement)
        }
      }
    }
  }
